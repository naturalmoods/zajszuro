...
const t = await (await fetch(`http://127.0.0.1:${port}/json/new?about:blank`, { method: "PUT" })).json();
  ...
  await sleep(400);
  await shot("4-speed");
  const speedState = await js(`({ view: document.body.className.match(/kvm-view-\\w+/g), calls: ${R}.querySelectorAll("#calls li").length, cost: ${R}.getElementById("cost").textContent, status: ${R}.getElementById("status").textContent })`);
  ...
  const p = pickState, b = baitState, k = backState;
    const costOk = speedState.calls > 0 && /Ft.*USD/.test(speedState.cost) && /Ft \(.*USD\)/.test(speedState.status) && /Ft \(.*USD\)/.test(p.hitsum);
    const ok = costOk && placeholder.startsWith("pl. ") && examples === 0 && topCount === Math.min(10, Number(itemCount)) && navigated && scored && searched && p.badgesVisible === 0 && p.dim > 0 && String(b.view) === "kvm-view-bait" && b.dimVisible === 0 && b.badgesVisible > 0 && String(speedState.view) === "kvm-view-bait" && String(k.view) === "kvm-view-pick" && k.badgesVisible === 0 && collapsed && reopened;
    if (!ok) process.exitCode = 1;
    console.log(ok ? "OK  " : "HIBA", url, "|", p.hitsum, "| pontszám látható:", b.badgesVisible, "| top:", topCount, "| költség:", speedState.status.split(" · ").pop(), "/", speedState.calls, "hívás", "| helyőrző:", placeholder, "| kattintás →", tabsAfter > tabsBefore ? "új lap" : now.slice(0, 70), "(link:", expected.slice(0, 50) + ")");
  ws.close();
